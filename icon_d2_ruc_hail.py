...
FILE_LAST_HOUR = "ultima_ora_icond2_ruc_hail.txt" 
RUN_DURATION = 27 
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_120")
    ...
    try:
        ...
        print(f"📸 Album Telegram GRANDINE (RUC) inviato ({len(file_paths)} mappe).")
    except Exception as e:
...
def genera_album_grandine(session: requests.Session, dt_run_utc: datetime, run_str: str):
    ...
    for block_name, ore_list in blocchi.items():
        ...
        if percorsi_foto:
            ...
            caption_album = f"ICON-D2-RUC EPS: Dimensione Max Grandine (Media membri, cm)\n{block_name}\nRun {nome_run}"
